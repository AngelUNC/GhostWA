...
func WatchMessages(msgDB *sql.DB, waDB *sql.DB) {
	for {
		...
		var msgs []types.WhatsAppMessage
		for rows.Next() {
			var (
		...
		// Procesar mensajes en orden ascendente
		for i := len(msgs) - 1; i >= 0; i-- {

			msg := msgs[i]

			current := MessageSnapshot{
				Text: db.StrOrEmpty(msg.TextData),
				Type: db.IntPtrToInt(msg.MessageType),
			}
			...
			if prev.Text != current.Text || prev.Type != current.Type {
				if current.Type == 15 || strings.EqualFold(current.Text, "(null)") {
					...
					fmt.Printf("%s - ✏️ Mensaje editado:\nOriginal: %s\nEditado: %s\n",
